...
#define COLOR_PALETTE_KEY (QStringLiteral("ColorPalette"))
    ...
    QObject(parent)
{
    ...
    SET_DEFAULT_VALUE(COLOR_PALETTE_KEY, QStringLiteral("purple"));
...
const QString Preferences::colorPalette() const
{
    return m_settings.value(COLOR_PALETTE_KEY).toString();
}

void Preferences::setColorPalette(const QString &palette)
{
    if(palette == colorPalette()) {
        return;
    }

    m_settings.setValue(COLOR_PALETTE_KEY, palette);
