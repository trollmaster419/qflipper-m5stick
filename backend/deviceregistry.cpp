...
// M5StickC Plus2 (ESP32 port) over its WCH CH343 USB-serial bridge.
// The firmware exposes a direct UART0<->RPC bridge, so it enumerates as a plain
// USB-serial device rather than a Flipper VCP.
#define M5_ESP32_VID 0x1a86
#define M5_ESP32_PID 0x55d4
    ...
    m_isQueryInProgress(false)
{
    ...
    m_detector->setWantedDevices({
        ...
        // M5StickC Plus2: match purely on VID/PID (the CH343 reports generic
        // manufacturer/product strings, so don't constrain on those).
        USBDeviceInfo(M5_ESP32_VID, M5_ESP32_PID)
                                 });
}
...
void DeviceRegistry::insertDevice(const USBDeviceInfo &info)
{
    ...
    } else if(info.vendorID() != FLIPPER_ZERO_VID && info.vendorID() != M5_ESP32_VID) {
