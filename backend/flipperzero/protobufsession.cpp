...
GuiSendInputOperation *ProtobufSession::guiSendAscii(int value)
{
    printf("\n>>> guiSendAscii received value: %d (0x%X) <<<\n", value, value);
    fflush(stdout);

    int mapped_key = -1;

    switch(value) {
        case 19: mapped_key = 0; break; // InputKeyUp
        case 17: mapped_key = 1; break; // InputKeyDown
        case 20: mapped_key = 2; break; // InputKeyRight
        case 18: mapped_key = 3; break; // InputKeyLeft
        case 13: case 10: mapped_key = 4; break; // InputKeyOk
        case 27: case 8: mapped_key = 5; break;  // InputKeyBack
        default: break;
    }

    if(mapped_key != -1) {
        // Send a single, clean Press (0) event. The firmware will handle the Release.
        return enqueueOperation(new GuiSendInputOperation(getAndIncrementCounter(), mapped_key, 2, this)); // 2 = Short
    }
