...
AbstractDeviceInfoHelper *AbstractDeviceInfoHelper::create(const USBDeviceInfo &info, QObject *parent)
{
    ...
    if(pid == 0x5740 || pid == 0x55d4) {
        // 0x55d4: M5StickC Plus2 ESP32 port — serial (VCP-like) RPC transport.
...
void VCPDeviceInfoHelper::fetchProtobufVersion()
{
    ...
    // Fail fast (instead of the 30s default) so a reconnect that lands on a
    // momentarily-poisoned bridge session is retried quickly rather than hanging.
    operation->setTimeout(3000);

    connect(operation, &AbstractOperation::finished, this, [=]() {
        if(operation->isError()) {
            if(m_protobufRetries-- > 0) {
                qCDebug(CATEGORY_DEBUG).noquote()
                    << QStringLiteral("protobuf_version failed (%1), retrying...").arg(operation->errorString());
                operation->deleteLater();
                // Re-issue on the same session; the device recycles its decoder
                // within ~1s, so the next attempt lands on a clean session.
                QTimer::singleShot(400, this, &VCPDeviceInfoHelper::fetchProtobufVersion);
                return;
            }
