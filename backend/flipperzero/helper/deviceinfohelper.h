...
private:
    ...
    // The M5Stick UART bridge can briefly eat the first protobuf_version after a
    // reconnect (it recycles its poisoned RPC session); retry a few times fast
    // instead of failing the whole device on a single 30s timeout.
    int m_protobufRetries = 5;
};
