...
USBDeviceInfo USBDeviceDetector::fillDeviceInfo(const USBDeviceInfo &deviceInfo)
{
    ...
    do {
        ...
        do {
            ...
            // Manufacturer/product strings are descriptive only. Some USB-serial
            // bridges (e.g. the WCH CH343 on the M5StickC Plus2) don't expose a
            // manufacturer string descriptor, so tolerate a missing/empty value
            // with a fallback instead of rejecting the device as "incomplete".
            status = libusb_get_string_descriptor_ascii(handle, desc.iManufacturer, buf, sizeof(buf));
            if(status < 0) {
                qCDebug(LOG_DETECTOR) << "No manufacturer string descriptor, using fallback:" << libusb_strerror((libusb_error)status);
                newinfo.setManufacturer(QStringLiteral("Unknown"));
            } else {
                const auto s = QString::fromLocal8Bit((const char*)buf);
                newinfo.setManufacturer(s.isEmpty() ? QStringLiteral("Unknown") : s);
            }

            status = libusb_get_string_descriptor_ascii(handle, desc.iProduct, buf, sizeof(buf));
            if(status < 0) {
                qCDebug(LOG_DETECTOR) << "No product string descriptor, using fallback:" << libusb_strerror((libusb_error)status);
                newinfo.setProductDescription(QStringLiteral("USB Serial"));
            } else {
                const auto s = QString::fromLocal8Bit((const char*)buf);
                newinfo.setProductDescription(s.isEmpty() ? QStringLiteral("USB Serial") : s);
            }
