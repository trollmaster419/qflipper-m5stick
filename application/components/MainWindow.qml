...
Item {
    ...
    RowLayout {
        ...
        ComboBox {
            ...
            readonly property var keys: ["purple", "amber", "green", "blue"]

            Layout.preferredWidth: 120
            Layout.fillHeight: true

            model: ["Purple", "Amber", "Green", "Blue"]

            currentIndex: Math.max(0, keys.indexOf(Preferences.colorPalette))
            onActivated: function(index) { Preferences.colorPalette = keys[index]; }

            ToolTip {
                visible: parent.hovered
                text: qsTr("Accent color palette (device illustration stays purple)")
                implicitWidth: 260
            }
        }
